...
eraser.addEventListener('click', () => {
  isEraser = true;
  brushIcon.style.background = 'pink';
...
function switchToBrush() {
  ...
  brushIcon.style.background = 'black';
  eraser.style.background = 'pink';
  currentColor = `#${brushColorBtn.value}`;
  currentSize = 10;
  brushSlider.value = currentSize;
  displayBrushSize();
...
function restoreCanvas() {
  for (let i = 1; i < drawnArray.length; i++) {
    ...
    if (drawnArray[i].erase) {
      console.log('what is this', drawnArray[i].erase)
...
downloadBtn.addEventListener('click', () => {
  ...
  downloadBtn.download = 'Image from canvas paint'
  // Active Tool
